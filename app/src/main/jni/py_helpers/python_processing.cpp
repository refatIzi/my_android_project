...
namespace py_helper {
    ...
    PythonProcessing::PythonProcessing() {
        ...
        __android_log_toviev("PythonProcessing Initialized");
    }

    PythonProcessing::~PythonProcessing() {
        __android_log_write(ANDROID_LOG_DEBUG, __FUNCTION__,
                            "We are unloading our PythonProcessing Object");
    ...
    int PythonProcessing::checkModule() {
        ...
        if (mPythonModule == nullptr) //??? nullptr == mPythonModule
        {
            __android_log_write(ANDROID_LOG_ERROR, __FUNCTION__,
                                "Python Module was not imported. Unable to process any python functions.");
            lReturn = -1;
        }
    ...
    int PythonProcessing::loadFile(string & aFileName) {
        if (nullptr != mPythonModule) {
            __android_log_write(ANDROID_LOG_WARN, __FUNCTION__,
                                "Python module has already been loaded, no need to load again.");
            return 0;
        }
        ...
        // Тож після кількох перевірок у нас має бути лише ім’я файлу, більше нічого
...
#pragma ide diagnostic ignored "UnusedGlobalDeclarationInspection"
    ...
    long PythonProcessing::executeFunction(const string &aFunctionName,
        ...
        if (!aFunctionName.empty()) {
            ...
            if (PyErr_Occurred()) {
                __android_log_print(ANDROID_LOG_ERROR, __FUNCTION__,
                                    "Error returned while building the python function object");
...
#pragma clang diagnostic pop
    ...
    long PythonProcessing::executeFunction(PyObject *aFunction, PyObject *aParameters) {
        ...
        if (lReturn != 0) {
            ...
            __android_log_print(ANDROID_LOG_ERROR, __FUNCTION__,
                                "Unable to call python executeFunction. An invalid function was passed.");
            return -1;
        }
        ...
        if (mPythonModule == nullptr) {
            // Somehow we got here with no Module. Bail
            //Якось ми потрапили сюди без модуля. Застава
            return -2;
        }
        ...
        if (PyLong_Check(lPythonReturnValue.mPyObject)) {
            ...
            if (PyErr_Occurred()) {
                __android_log_print(ANDROID_LOG_ERROR, __FUNCTION__,
                                    "Error returned while calling getting return value for call");
                return handlePythonError();
            }
        }
    ...
    int PythonProcessing::getStringFromPyObject(PyObject *aPyObject, string &aReturnString) {
        if (aPyObject == nullptr) {
            //__android_log_write(4,_ctype_,"");
            __android_log_write(ANDROID_LOG_ERROR, __FUNCTION__,
                                "Unable to get the string from a null object.");
            return -101;
        }
        ...
        string str = "GET String Python " + aReturnString;
        __android_log_write(ANDROID_LOG_ERROR, __FUNCTION__, str.c_str());
        log.LOG("Compile");
        //error_string = "Check error";
        __android_log_toviev(aReturnString);


        if (PyErr_Occurred()) {
            __android_log_write(ANDROID_LOG_ERROR, __FUNCTION__,
                                "Error returned getting string from object");
            handlePythonError();
            return -110;
        }
        return 0;
    ...
    int PythonProcessing::getVectorFromPyObject(PyObject *aPyObject,
        ...
        if (aPyObject == nullptr) {
            __android_log_print(ANDROID_LOG_ERROR, __FUNCTION__,
                                "Unable to get the vector from a null dictionary object.");
            return -101;
        }
            // Verify that it's a Tuple
        else if (!PyList_Check(aPyObject)) {
            __android_log_print(ANDROID_LOG_ERROR, __FUNCTION__,
                                "Python object is not a Dictionary, and can not be converted to a vector.");
            return -102;
        }
        ...
        if (PyErr_Occurred()) {
            __android_log_print(ANDROID_LOG_ERROR, __FUNCTION__,
                                "Error returned while calling PyList_Size");
            handlePythonError();
            return -103;
        } else if (lListSize > 0) {
            aItems.clear();
        ...
        for (Py_ssize_t lLoop = 0; lLoop < lListSize; ++lLoop) {
            ...
            if (PyErr_Occurred()) {
                __android_log_print(ANDROID_LOG_ERROR, __FUNCTION__,
                                    "Error returned while calling PyList_GetItem");
                handlePythonError();
                return -105;
            }
            ...
            if (lReturn != 0) {
                __android_log_print(ANDROID_LOG_WARN, __FUNCTION__,
                                    "Error getting string from py object.");
            }
