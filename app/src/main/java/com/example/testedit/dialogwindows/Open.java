...
public class Open implements AdapterView.OnItemClickListener {
        ...
        for (int i = 0; i < sDirList.length; i++) {
            ...
            if (f1.isFile()) {
                if (sDirList[i].endsWith(".py")) {
                    mLista.add(new Search(sDirList[i], DataSaving.getTime(f1).toString().replaceAll("T1|T2|Z", "\n") + "", DataSaving.readInformation(sDirList[i], "information", aDirectory), R.drawable.ic_filepython, false));
                } else {
                }
            } else {
                if (sDirList[i].endsWith("project"))
                    mLista.add(new Search(sDirList[i], DataSaving.getTime(f1).toString().replaceAll("T1|T2|Z", "\n") + "", DataSaving.readInformation(sDirList[i], "information", aDirectory), R.drawable.ic_python_prog, false));
                else
                    mLista.add(new Search(sDirList[i], DataSaving.getTime(f1).toString().replaceAll("T1|T2|Z", "\n") + "", DataSaving.readInformation(sDirList[i], "information", aDirectory), R.drawable.ic_folder_python_3, false));
            }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        if (aDirectory.isFile()) {
            mainInterface.setEditText(DataSaving.readInformation(mAdapter.getItem(position).getNomber(), "readCode", Directory));
